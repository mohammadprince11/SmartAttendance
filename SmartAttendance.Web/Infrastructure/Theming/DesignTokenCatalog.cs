...
public static class DesignTokenCatalog
{
    ...
    public static readonly IReadOnlyList<Token> All = new[]
    {
        ...
        // ---- بطاقة الإحصاء (KPI) ----
        //
        // ⚠️ عائلةٌ كانت **خارج الحوكمة تماماً**: قِيست حيّاً على 24 صفحة فوُجد
        // **اثنتا عشرة عائلة** لنفس العنصر بارتفاعات 56 ← 104px وأرقام 18 ← 28px
        // وانحناءات 12 ← 20px (`zhr-kpi-card` · `zy-dash__kpi` · `hrms-stat` ·
        // `nxr-doc-kpi` · `zxd-kpi` · `tx-kpi` · `ln-kpi` · `mp-kpi` · `rv-kpi` ·
        // `apv-kpi` · `zua-stat` …). أي أن أضخمها ضِعف أصغرها لعرض رقمٍ واحد.
        // الافتراضات هنا **مضغوطة عمداً** لتناسب كثافة النظام (تحكّم 30px،
        // نصّ 13px) بدل الكتل السابقة.
        // الافتراضات **8/14/12/18/11** باختيار محمد 2026-08-02 («اعتمد الصغير»):
        // أصغر ممّا اقترحتُه (10/…/20) وبأرقامٍ مقصودة. وسبب توحيد الافتراض هنا
        // مع المعتمَد بالقاعدة: لو اختلفا لبقي الكودُ يقول شيئاً والشاشةُ شيئاً،
        // ولوجد من يقرأ CSS رقماً لا يراه بالمتصفّح.
        new Token("kpi-pad-y", "حشوة بطاقة الإحصاء (رأسية)", GroupSurfaces, 8, 4, 24),
        new Token("kpi-pad-x", "حشوة بطاقة الإحصاء (أفقية)", GroupSurfaces, 14, 6, 28),
        new Token("kpi-radius", "استدارة بطاقة الإحصاء", GroupSurfaces, 12, 0, 28),
        new Token("kpi-value-font", "رقم بطاقة الإحصاء", GroupSurfaces, 18, 12, 34, 0.5),
        new Token("kpi-label-font", "عنوان بطاقة الإحصاء", GroupSurfaces, 11, 8, 18, 0.5),
