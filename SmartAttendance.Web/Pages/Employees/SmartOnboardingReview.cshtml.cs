...
public sealed class SmartOnboardingReviewModel : PageModel
{
    ...
    public async Task<IActionResult> OnPostFinalizeAsync(int departmentSelectionId)
    {
        ...
        // SP12 TEMP DIAGNOSTICS START
        // Numeric form/model-binding diagnostics only. No names, document
        // numbers, national IDs, emails, phones, or document content are logged.
        var diagnosticLogger =
            HttpContext.RequestServices.GetService(
                typeof(Microsoft.Extensions.Logging.ILogger<SmartOnboardingReviewModel>))
            as Microsoft.Extensions.Logging.ILogger<SmartOnboardingReviewModel>;

        var postedDepartmentId =
            Request.HasFormContentType
                ? Request.Form["Finalize.DepartmentId"]
                : Microsoft.Extensions.Primitives.StringValues.Empty;

        var postedDepartmentSelectionId =
            Request.HasFormContentType
                ? Request.Form["departmentSelectionId"]
                : Microsoft.Extensions.Primitives.StringValues.Empty;

        var postedBranchId =
            Request.HasFormContentType
                ? Request.Form["Finalize.BranchId"]
                : Microsoft.Extensions.Primitives.StringValues.Empty;

        ModelState.TryGetValue(
            "Finalize.DepartmentId",
            out var departmentModelState);

        var modelStateRawValue =
            departmentModelState?.RawValue is string[] rawValues
                ? string.Join("|", rawValues)
                : System.Convert.ToString(
                    departmentModelState?.RawValue,
                    System.Globalization.CultureInfo.InvariantCulture)
                    ?? string.Empty;

        var modelStateErrors =
            departmentModelState is null
                ? string.Empty
                : string.Join(
                    " | ",
                    departmentModelState.Errors.Select(error =>
                        !string.IsNullOrWhiteSpace(error.ErrorMessage)
                            ? error.ErrorMessage
                            : error.Exception?.GetType().Name
                                ?? "UnknownModelStateError"));

        var diagnosticDepartmentId =
            departmentSelectionId > 0
                ? departmentSelectionId
                : Finalize.DepartmentId;

        SmartAttendance.Application.Departments.ViewModels.DepartmentListViewModel?
            diagnosticDepartment = null;

        if (diagnosticDepartmentId > 0)
        {
            diagnosticDepartment = (await _employeeService
                    .GetDepartmentsForDropdownAsync(
                        CompanyId,
                        context.Value.Access.Scope))
                .FirstOrDefault(x => x.Id == diagnosticDepartmentId);
        }

        diagnosticLogger?.LogWarning(
            "ZYNORA_SP12_DEPARTMENT_POST " +
            "PostedDepartmentId={PostedDepartmentId} " +
            "PostedDepartmentIdCount={PostedDepartmentIdCount} " +
            "PostedDepartmentSelectionId={PostedDepartmentSelectionId} " +
            "PostedDepartmentSelectionIdCount={PostedDepartmentSelectionIdCount} " +
            "BoundFinalizeDepartmentId={BoundFinalizeDepartmentId} " +
            "PostedBranchId={PostedBranchId} " +
            "PostedBranchIdCount={PostedBranchIdCount} " +
            "BoundFinalizeBranchId={BoundFinalizeBranchId} " +
            "ModelStateRawValue={ModelStateRawValue} " +
            "ModelStateAttemptedValue={ModelStateAttemptedValue} " +
            "ModelStateErrorCount={ModelStateErrorCount} " +
            "ModelStateErrors={ModelStateErrors} " +
            "SelectedDepartmentId={SelectedDepartmentId} " +
            "SelectedDepartmentExists={SelectedDepartmentExists} " +
            "SelectedDepartmentBranchId={SelectedDepartmentBranchId} " +
            "SelectedDepartmentCompanyId={SelectedDepartmentCompanyId} " +
            "SelectedDepartmentIsActive={SelectedDepartmentIsActive}",
            postedDepartmentId.ToString(),
            postedDepartmentId.Count,
            postedDepartmentSelectionId.ToString(),
            postedDepartmentSelectionId.Count,
            Finalize.DepartmentId,
            postedBranchId.ToString(),
            postedBranchId.Count,
            Finalize.BranchId,
            modelStateRawValue,
            departmentModelState?.AttemptedValue ?? string.Empty,
            departmentModelState?.Errors.Count ?? 0,
            modelStateErrors,
            diagnosticDepartmentId,
            diagnosticDepartment is not null,
            diagnosticDepartment?.BranchId,
            diagnosticDepartment?.CompanyId,
            diagnosticDepartment?.IsActive);
        // SP12 TEMP DIAGNOSTICS END
