...
public partial class ProfileModel
{
    ...
    {
        TimelineTotalCount = await HrmsDatabase.ScalarAsync<int>(
            _dbContext,
            """
SELECT COUNT(*) FROM
(
...
""",
        ...
        TimelinePage = NormalizeActivityPage(TimelinePage, TimelineTotalCount);
        var offset = (TimelinePage - 1) * ActivityPageSize;

        TimelineEvents = await HrmsDatabase.QueryAsync(
            _dbContext,
            """
SELECT EventDate, Kind, Title, Detail FROM
(
    SELECT CAST(e.HireDate AS datetime2) AS EventDate, 'hire' AS Kind,
           N'تعيين' AS Title, N'انضم الموظف للشركة' AS Detail
    FROM Employees e WHERE e.Id = @Id
