...
namespace SmartAttendance.Web.Pages.PositionCategories;
...
public class IndexModel : PageModel
{
    ...
    public async Task<IActionResult> OnPostSaveAsync()
    {
        ...
        {
            TempData["ErrorMessage"] = "اسم الفئة مطلوب.";
...
SELECT COUNT(1)
FROM dbo.HrJobPositionCategories
...
", command =>
        ...
        if (duplicateCount > 0)
        {
            TempData["ErrorMessage"] = "اسم الفئة موجود مسبقاً.";
            await LoadPageAsync(Input.Id > 0 ? Input.Id : null);
            return Page();
        }
...
UPDATE dbo.HrJobPositionCategories
SET Name = @Name,
    IsActive = @IsActive,
...
", command =>
            ...
            TempData["SuccessMessage"] = "تم حفظ الفئة بنجاح.";
        }
        else
        {
            await ExecuteNonQueryAsync(@"
...
", command => AddParameter(command, "@Id", id));
        ...
        TempData["SuccessMessage"] = "تم تحديث حالة الفئة.";
        return RedirectToPage();
    }
    ...
    public async Task<IActionResult> OnPostDeleteAsync(int id)
    {
        ...
        if (usageCount > 0)
        {
            TempData["ErrorMessage"] = "لا يمكن حذف فئة مستخدمة بمنصب.";
            return RedirectToPage();
        }

        await ExecuteNonQueryAsync(@"
DELETE FROM dbo.HrJobPositionCategories
WHERE Id = @Id;
", command => AddParameter(command, "@Id", id));
...
WHERE p.Category IS NOT NULL
  ...
  (
      SELECT 1
      FROM dbo.HrJobPositionCategories item
      WHERE LTRIM(RTRIM(item.Name)) = LTRIM(RTRIM(p.Category))
  );
");
    }
...
SELECT
    ...
    (
        ...
        WHERE LTRIM(RTRIM(ISNULL(position.Category, N''))) = LTRIM(RTRIM(item.Name))
    ) AS UsageCount
FROM dbo.HrJobPositionCategories item
ORDER BY item.Name;";
...
WHERE LTRIM(RTRIM(ISNULL(Category, N''))) = @Name;
...
SET Category = @NewName,
    UpdatedAt = SYSDATETIME()
WHERE LTRIM(RTRIM(ISNULL(Category, N''))) = @OldName;
", command =>
        {
