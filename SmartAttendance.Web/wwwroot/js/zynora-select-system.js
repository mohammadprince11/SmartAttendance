/* ZYNORA_FIX15C_GLOBAL_SELECT_SYSTEM_START */
(function () {
    "use strict";
    ...
    function isNativeOnly(select) {
        ...
        if (select.dataset.zynoraNativeOnly === "true") return true;
        if (select.dataset.zynoraNative === "true") return true;
        if (select.classList.contains("nxcs-skip")) return true;
        if (select.classList.contains("zynora-native-only")) return true;
        if (closest(select, "[data-zynora-native-selects='true']")) return true;
        return false;
    }
    ...
    function selectPanelMinWidth(select) {
        var raw = select && select.dataset
            ? select.dataset.zynoraSelectPanelMinWidth
            : "";
    ...
    window.ZynoraRefreshSelectSystem = refreshAll;
})();
/* ZYNORA_FIX15C_GLOBAL_SELECT_SYSTEM_END */
