...
| 22·23 | ✅ **بوّابة CI للعزل — سقّاطة `TenantGuardRatchetTests`**: تمسح كل نماذج الصفحات وتفشل على أي صفحة **جديدة** تقبل معرّفاً من الطلب بلا حارس نطاق (النمط ذاته الذي أنتج كل ثغرات العبور)، وتفشل أيضاً على أي مُدخَل خط أساس صار محروساً أو حُذف — فالسقّاطة تشدّ ولا ترخي. تعمل بالـCI القائم بلا قاعدة بيانات. **خط الأساس = 39 صفحة دَين موروث (لم تُدقَّق، ليست «آمنة») ⟸ مسح Phase 21.** المصفوفة الحيّة (شركة A ضد موارد B بمتصفّح) تبقى: تلزمها جلسة دخول | جزئيّ — البوّابة مُغلقة |
